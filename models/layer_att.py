...
class GNN_nl(nn.Module):
    def __init__(self, input_features, nf, n_way=5):
        ...
        for i in range(self.num_layers):
            if i == 0:
                ...
                in_feat = int(self.nf / 2)
                
            module_w = Wcompute(in_feat, nf, operator='J2', activation='softmax', ratio=[2, 2, 1, 1])
            module_l = Gconv(in_feat, int(nf / 2), 2)
                
            self.add_module('layer_w{}'.format(i), module_w)
            self.add_module('layer_l{}'.format(i), module_l)

        self.w_comp_last = Wcompute(int(self.nf / 2), nf, operator='J2', activation='softmax', ratio=[2, 2, 1, 1])
        self.layer_last = Gconv(int(self.nf / 2), self.train_N_way, 2, bn_bool=False)
    ...
    def forward(self, x):
        ...
        for i in range(self.num_layers):
            ...
            # ABLATION: Removing dense connection
            # torch.cat([x, x_new], 2) is replaced with just the new features.
            x = x_new
